...
int
onionroute_closestream_i(void *conn)
{
  entry_connection_t *ap_conn=NULL;
  ...
  ap_conn = TO_ENTRY_CONN(conn);

  connection_mark_unattached_ap(ap_conn, END_STREAM_REASON_DONE);
  send_control_done(conn);
  return 0;
}

typedef struct onionroute_close_command_t
{

  void *id;

} onionroute_close_command_t;

void
close_command_processor(void *data)
{
	void *id;

	onionroute_close_command_t *cmd;

	cmd = (onionroute_close_command_t *)data;

	id = cmd->id;

	onionroute_closestream_i(id);
}
...
{
	onionroute_close_command_t *ccmd;
	onionroute_command_t *cmd;

	/* store data */
	ccmd = tor_malloc(sizeof(onionroute_close_command_t));

	if(NULL == ccmd) { return -1; }
	...
	cmd->data = ccmd;
	cmd->processor = close_command_processor;
