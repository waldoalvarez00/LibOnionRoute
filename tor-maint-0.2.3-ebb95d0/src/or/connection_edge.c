...
#ifdef LIBRARY
/*	if(edge_conn->is_onionroute_request)
	{
		if(closecallback)
		{
			closecallback(edge_conn);
		}
	}*/
#endif
...
void onionroute_clear_dns_cache_signal_i()
{
	addressmap_get_mappings(NULL, 2, TIME_MAX, 0);
}

void
	clear_dns_command_processor(void *data)
{
	onionroute_clear_dns_cache_signal_i();
}

int
	onionroute_clear_dns_cache_signal()
{
	...
	cmd->data = NULL;
	cmd->processor = clear_dns_command_processor;
