...
#ifdef LIBRARY
  tor_mutex_acquire(connection_array->lock);
#endif
...
#ifdef LIBRARY
  tor_mutex_release(connection_array->lock);
#endif
...
#ifdef LIBRARY
void
onionroute_switch_to_new_circuits_i(time_t now)
{
...
void
switch_to_new_circuits_command_processor(void* data)
{
	onionroute_switch_to_new_circuits_i(time(NULL));
}

ONIONROUTE_API
int onionroute_switch_to_new_circuits_v1()
{
	...
	cmd->data = NULL;
	cmd->processor = switch_to_new_circuits_command_processor;
...
int onionroute_init_v1()
{
	...
	log_set_application_name("libonionroute");
	/* Initialize the history structures. */
	...
	options_init_from_string(/*const char *cf_defaults*/ "", /*const char *cf*/ "",
		/*int command*/ CMD_RUN_TOR, /*const char *command_arg*/ "", /*char **msg*/ &msg);



#ifndef _WIN32
		if (geteuid()==0)
			log_warn(LD_GENERAL,"You are running libonionroute as root. You don't need to, "
			"and you probably shouldn't.");
#endif
